									...
									if (response.data.error.length === 0) {
										...
										db.transaction(tx => {
											response.data.workidbackend.every(id => {
												tx.executeSql('Update USER_WORKS set workid_backend=? where workid=?', [id.workid_backend, id.workid], () => { console.log('update completed'); }, (_, error) => { console.log('please error batado', error) });
...
const dataupdatedfunction = (userid, setworklocaldata, handlerefresh) => {
...
export default function Updatedatatocloud(userid, setworklocaldata = (() => { }), handlerefresh = (() => { })) {
	...
	NetInfo.fetch().then(state => {
		...
		dataupdatedfunction(userid, setworklocaldata, handlerefresh);
		}
	});
}
