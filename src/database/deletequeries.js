...
export const deletework = (workid, completed, sortBy, defaultworkid, data, workidbackend) => {
  console.log(
...
export const deleteTask = (taskids) => {
  return (dispatch) => {
     db.transaction(tx => {
          taskids.forEach(taskid => {
            tx.executeSql('delete from work_tasks where taskid=?', [taskid], () => {
              ...
              tx.executeSql('Insert into TASK_DATA_UPDATE(update_type, workid, taskid) values(?,?,?)',['DELETE',workid, taskid],() => {
                console.log('----data update successfuly inserted into TASK_DATA_UPDATE TABLE-----');
              }, (error) => {
               ...
               dispatch({
                type: DELETE_SELECTED_TASK_FAIL,
                payload: error
              })
            ));
        });
        ...
        return dispatch({
          type: DELETE_SELECTED_TASK_SUCCESS,
          payload: 'Task successfuly deleted'
        });
      });
  };
};
