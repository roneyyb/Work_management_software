...
export default class Datetimemodal extends Component {
  ...
  configure() {
    PushNotification.configure({
      ...
      onNotification: () => {console.log('notifiction set')},
  permissions: {
        alert: true,
        badge: true,
        sound: true
  ...
  openAndroidTimePicker = async () => {
    ...
    try {
      ...
      if (action !== TimePickerAndroid.dismissedAction) {
        ...
        return [`${hours}:${minute} ${newformat}`, hour, minute];
  ...
  Returnscreen = () => {
    if(this.state.Modalscreen === 0)
    {
      return(
      <Deadline
      ref = {child => {this.returnscreen = child;}}
  ...
  onChange = (event,date) => {
    ...
    hours = hours ? hours : 12;  
      this.setOpacity(1);
      if(hours < 10 && minute<10)
      this.returnscreen.setDeadline([`0${hours}:0${minute} ${newformat}`, hour, minute]);
      else if(hours<10 && minute>10)
      this.returnscreen.setDeadline([`0${hours}:${minute} ${newformat}`, hour, minute]);
      else if(hours>10 && minute<10)
      this.returnscreen.setDeadline([`${hours}:0${minute} ${newformat}`, hour, minute]);
      else
      this.returnscreen.setDeadline([`${hours}:${minute} ${newformat}`, hour, minute]);
    }
  }
  render() {
    return (
